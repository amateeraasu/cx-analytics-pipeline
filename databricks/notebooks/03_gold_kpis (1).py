...
# MAGIC **Source:** `workspace.olist_silver` Delta tables
# MAGIC **Output:** 3 managed Delta tables in `workspace.olist_gold`
# MAGIC
...
SILVER_DB  = "workspace.olist_silver"
GOLD_DB    = "workspace.olist_gold"

spark.sql(f"CREATE SCHEMA IF NOT EXISTS {GOLD_DB}")
print(f"Source : {SILVER_DB}")
print(f"Target : {GOLD_DB}")
...
def write_gold(df, table_name: str) -> int:
    """Write a DataFrame to a managed Gold Delta table in Unity Catalog."""
    ...
    print(f"  \u2713  {full_table_name:55s} {n:>6,} rows")
    return n
...
geo = (
    spark.table("workspace.olist_bronze.raw_geolocation")
    .groupBy("city", "state")
    .agg(
...
orders_with_state = (
    ...
    .filter(F.col("order_status") == "delivered")
    .join(
        spark.table("workspace.olist_bronze.raw_customers").select("customer_id", "customer_unique_id"),
        on="customer_id",
        how="left",
    )
...
# MAGIC FROM workspace.olist_gold.gold_monthly_kpis
...
# MAGIC FROM workspace.olist_gold.gold_delivery_by_state
...
# MAGIC FROM workspace.olist_gold.gold_customer_segments
...
# MAGIC
# MAGIC ```
# MAGIC CSV files (UC Volume: /Volumes/workspace/olist_bronze/raw_files/)
# MAGIC     │
# MAGIC     ▼
# MAGIC workspace.olist_bronze  ← 01_bronze_ingest.py
# MAGIC     │  9 raw Delta tables, metadata columns added
# MAGIC     ▼
# MAGIC workspace.olist_silver  ← 02_silver_transform.py
# MAGIC     │  silver_orders / silver_customers / silver_churn_features
# MAGIC     │  Delivery KPIs, customer aggregations, ML features
# MAGIC     ▼
# MAGIC workspace.olist_gold    ← 03_gold_kpis.py  (this notebook)
