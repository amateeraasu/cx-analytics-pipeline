...
print(f"Source : {BRONZE_DB}")
print(f"Target : {SILVER_DB}")
...
def write_silver(df, table_name: str) -> int:
    """Write a DataFrame to a managed Silver Delta table in Unity Catalog."""
...
silver_customers = (
    ...
    .agg(
        # Most recent canonical address (rn=1 trick)
        F.first(
            F.when(F.col("rn") == 1, F.col("state")),
            ignorenulls=True
        ).alias("state"),
        F.first(
            F.when(F.col("rn") == 1, F.col("city")),
            ignorenulls=True
        ).alias("city"),
